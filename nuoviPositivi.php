<?php
  ...
  for($i = 11; $i > 0; $i--) {
    ...
    $positivi[$handler] = $datiLetti[1][6];
    $handler++;
  }

  for($i = 0; $i < 10; $i++) {
    $calcolato = $positivi[$i + 1] - $positivi[$i];
    $nuoviPositivi[$i] = array("y" => $calcolato, "label" => $dataDati[$i + 1]);
  }
    ...
    window.onload = function() {
      var chart = new CanvasJS.Chart("chartContainer", {
        ...
        data: [{
          type: "column",
          name: "Deceduti",
          legendText: "Deceduti nella giornata",
          showInLegend: true,
          dataPoints:<?php echo json_encode($nuoviPositivi, JSON_NUMERIC_CHECK); ?>
        }]
      });
      chart.render();
  ...
  <div class="mdl-layout mdl-js-layout mdl-layout--fixed-drawer
    ...
    <header class="mdl-layout__header">
      <div class="mdl-layout__header-row">
        <span class="mdl-layout-title">Nuovi positivi</span>
      </div>
    </header>
